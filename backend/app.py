...
def rtc_transcribe_connect():
    ...
    return resp
# ================== PERSONALIZED MEDICINE (NUDGE) ORCHESTRATOR ==================

# In-memory store for nudge sessions
# nudge_sessions[session_id] = {
#   "complaint": str,
#   "answers": {"age_years":..., "biological_sex":..., "pregnancy_status":..., "weight_kg":..., "<custom>":...},
#   "history": [{"questionId": str, "text": str, "answer": Any}],
#   "asked_ids": set([...]),
#   "steps": int
# }
nudge_sessions: Dict[str, Dict[str, Any]] = {}

# Helper: coerce to float / bool
def _coerce_bool(x):
...
def _nudge_get(session_id: str) -> Dict[str, Any]:
    nudge_sessions.setdefault(session_id, {
        "complaint": "",
        "answers": {},
        "history": [],
        "asked_ids": set(),
    ...
    return nudge_sessions[session_id]

def _nudge_reset(session_id: str):
    if session_id in nudge_sessions:
        del nudge_sessions[session_id]

# Map incoming answer ids to canonical keys we also store in `session_context`
_CANON = {
    "age": "age_years", "age_years": "age_years",
    "sex": "biological_sex", "biological_sex": "biological_sex",
    "pregnancy": "pregnancy_status", "pregnancy_status": "pregnancy_status",
    "weight": "weight_kg", "weight_kg": "weight_kg",
}

def _merge_answers(session_id: str, answers: Dict[str, Any]):
    sess = _nudge_get(session_id)
    for k, v in (answers or {}).items():
        key = _CANON.get(k, k)
        sess["answers"][key] = v

def _need_pregnancy_q(answers: Dict[str, Any]) -> bool:
    sex = (answers.get("biological_sex") or "").lower()
    ...
    try:
        if age is None: 
            return True
        # ask within broad reproductive window
        return 10 <= float(age) <= 55
    except Exception:
        return True

# Fixed critical questions (small set; minimal friction)
def _next_fixed_critical_question(answers: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    if answers.get("age_years") in (None, ""):
        return {
            "id": "age_years",
            "text": "What is the patient's age (years)?",
            "type": "number",
            "unit": "years"
        }
    if (answers.get("biological_sex") or "") == "":
        return {
            "id": "biological_sex",
            "text": "What is the patient's biological sex?",
            "type": "enum",
            "options": ["male", "female", "other"]
        }
    if answers.get("pregnancy_status") in (None, "") and _need_pregnancy_q(answers):
        return {
            "id": "pregnancy_status",
    ...
    if answers.get("weight_kg") in (None, ""):
        return {
            "id": "weight_kg",
            "text": "What is the patient's weight (kg)?",
            "type": "number",
            "unit": "kg"
        }
    return None

# Build prompt to request ONE best next question + short analysis
def _build_next_question_prompt(complaint: str, answers: Dict[str, Any], history: List[Dict[str, Any]]) -> str:
    return (
        "You are a clinical intake assistant. Based on the short complaint and answers so far, "
        "propose the SINGLE most critical next question to clarify diagnosis or immediate risk. "
        "If enough information is gathered for a preliminary working diagnosis OR if urgent red flags are present, set stop=true. "
        "Keep it short, patient-friendly, and high yield.\n\n"
        "Return STRICT JSON with EXACT keys:\n"
        "{\n"
        '  "next": {"id":"string","text":"string","type":"enum|boolean|number|text","options":[],"unit":null},\n'
        ...
        "}\n\n"
        "Rules:\n"
        "- NEVER repeat questions already asked (see history).\n"
        "- Keep options <= 6 when type=enum; use lowercase generic phrasing.\n"
        "- When type=number, provide a sensible unit in 'unit' or null.\n"
        "- Prefer boolean or enum when a binary/short list exists.\n"
        "- Do not ask administrative or billing questions.\n\n"
        f"Complaint: {complaint}\n"
        f"Answers so far: {json.dumps(answers, ensure_ascii=False)}\n"
        f"History (asked): {json.dumps(history, ensure_ascii=False)}\n"
    )

# Call LLM to get next question proposal
def _llm_next_question(complaint: str, answers: Dict[str, Any], history: List[Dict[str, Any]]) -> Dict[str, Any]:
    prompt = _build_next_question_prompt(complaint, answers, history)
    try:
        ...
        if not isinstance(nxt, dict):
            nxt = {}
        nxt.setdefault("type", "text")
        nxt.setdefault("id", f"q_{len(history)+1}")
        nxt.setdefault("text", "Provide more detail about the main complaint.")
        nxt.setdefault("options", [])
        nxt.setdefault("unit", None)

        alts = data.get("alternatives") or []
        if not isinstance(alts, list): alts = []
        analysis = data.get("analysis") or {}
        if not isinstance(analysis, dict): analysis = {}
        stop = bool(data.get("stop")) if isinstance(data.get("stop"), bool) else False

        return {"next": nxt, "alternatives": alts[:2], "analysis": analysis, "stop": stop}
    except Exception as e:
        # fallback generic
        return {
            "next": {
                "id": f"q_{len(history)+1}",
                "text": "When did the symptoms start?",
                "type": "text",
                "options": [],
                "unit": None
            },
            "alternatives": [{"id": "q_alt1", "text": "What makes the symptoms better or worse?"}],
            "analysis": {},
            "stop": False
        }

# Decide state from analysis / steps
def _decide_state(analysis: Dict[str, Any], steps: int, max_steps: int = 6) -> str:
    red_flags = [*(analysis.get("red_flags") or [])]
    if any(isinstance(x, str) and x.strip() for x in red_flags):
...
def _build_payload(sess: Dict[str, Any], nxt_pack: Optional[Dict[str, Any]], state: str) -> Dict[str, Any]:
    return {
        "state": state,  # "ask" | "complete" | "triage_urgent"
        "next_question": (nxt_pack or {}).get("next"),
        "alternatives": (nxt_pack or {}).get("alternatives", []),
        "analysis": (nxt_pack or {}).get("analysis", {}),
        "collected": {
...
# ================== END PERSONALIZED MEDICINE (NUDGE) ORCHESTRATOR ==============
# ===== Personalized Medicine: get NEXT question =====
@app.route("/nudge-next", methods=["POST"])
def nudge_next():
    data = request.get_json() or {}
    session_id = data.get("session_id", str(uuid4()))
    complaint = (data.get("complaint") or "").strip() or "Consultation"
    incoming_answers = data.get("answers") or {}
    history_in = data.get("history") or []

    sess = _nudge_get(session_id)
    # Seed/merge session data
    if not sess.get("complaint"):
        sess["complaint"] = complaint
    _merge_answers(session_id, incoming_answers)

    # also adopt any caller-provided history (optional)
    for h in history_in:
        try:
            qid = str(h.get("questionId"))
            txt = str(h.get("text") or "")
            ans = h.get("answer")
        except Exception:
            continue
        if qid and qid not in sess["asked_ids"]:
            sess["history"].append({"questionId": qid, "text": txt, "answer": ans})
            sess["asked_ids"].add(qid)

    # 1) Fixed critical questions first
    fixed_q = _next_fixed_critical_question(sess["answers"])
    if fixed_q:
        # Don’t count fixed questions toward max_steps until answered
        payload = {
            "state": "ask",
            "next_question": fixed_q,
            "alternatives": [],
            "analysis": {},
            "collected": {
    ...
    # 2) LLM-driven next question (short series)
    nxt = _llm_next_question(sess["complaint"], sess["answers"], sess["history"])
    # Decide state (stop/triage vs continue)
    state = "complete" if nxt.get("stop") else _decide_state(nxt.get("analysis") or {}, sess["steps"])
    payload = _build_payload(sess, nxt, state)
    return jsonify(payload), 200


# ===== Personalized Medicine: POST an answer, advance the flow =====
@app.route("/nudge-answer", methods=["POST"])
def nudge_answer():
    data = request.get_json() or {}
    session_id = data.get("session_id", str(uuid4()))
    complaint = (data.get("complaint") or "").strip() or "Consultation"
    ...
    # Map canonical keys and store answer
    canon = _CANON.get(qid, qid)
    if canon in ("age_years", "weight_kg"):
        try:
            ans = float(ans)
        except Exception:
            pass
    if canon == "pregnancy_status":
    ...
    sess["answers"][canon] = ans
    sess["history"].append({"questionId": qid, "text": "", "answer": ans})
    sess["asked_ids"].add(qid)
    sess["steps"] += 1  # count only after answers come in

    # 1) If any fixed critical question remains, ask it next without consuming step
    fixed_q = _next_fixed_critical_question(sess["answers"])
    if fixed_q:
        payload = {
            "state": "ask",
    ...
    # 2) Otherwise ask LLM for the single best next question or stop
    nxt = _llm_next_question(sess["complaint"], sess["answers"], sess["history"])
    state = "complete" if nxt.get("stop") else _decide_state(nxt.get("analysis") or {}, sess["steps"])

    # If flow is complete or triage is suggested, persist structured context into your strict store
    if state in ("complete", "triage_urgent"):
        # Push into session_context to benefit downstream dosage/summary endpoints
        session_context.setdefault(session_id, {})
        ctx = session_context[session_id]
        # merge canonical fields
        if sess["answers"].get("age_years") is not None:
            ctx["age_years"] = sess["answers"]["age_years"]
        if sess["answers"].get("weight_kg") is not None:
            ctx["weight_kg"] = sess["answers"]["weight_kg"]
        if sess["answers"].get("biological_sex"):
            ctx["biological_sex"] = sess["answers"]["biological_sex"]
        if sess["complaint"]:
            ctx["condition"] = ctx.get("condition") or sess["complaint"]
...
@app.route("/nudge-reset", methods=["POST"])
def nudge_reset():
    data = request.get_json() or {}
    session_id = data.get("session_id", "")
    if not session_id:
        return jsonify({"error": "Missing session_id"}), 400
    _nudge_reset(session_id)
