...
import ChatBubbleChart from './ChatBubbleChart';
...
const Chat = () => {
  ...
  const handleNewMessage = async ({ text, skipEcho = false }) => {
    ...
    if (wantsPieChart(text)) {
      try {
        // If you already have a parsed differential, pass it instead of context:
        // const latestDiff = chats.findLast(c => c?.type === 'secondOpinion')?.opinion?.differential_diagnosis;
        // const config = await requestPieFromDifferential({ sessionId, differential: latestDiff, title: 'Differential diagnosis' });
      ...
      return; // don't also send this to /stream
    }
