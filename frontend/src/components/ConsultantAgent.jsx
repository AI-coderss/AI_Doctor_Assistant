...
const ConsultantAgent = forwardRef(function ConsultantAgent(
    ...
    function routeTool(name, args) {
        ...
        if (name === "emit_assessment") {
            ...
            try {
                window.dispatchEvent(new CustomEvent("ddx:render", { detail: { items: ddx } }));
            } catch { }
        ...
        if (name === "emit_ddx") {
            const ddx = Array.isArray(args?.ddx) ?
                args.ddx : [];
        ...
        if (name === "share_open_widget") {
            try {
                window.dispatchEvent(new CustomEvent("sw:open", { detail: { recipient_hint: args?.recipient_hint || "" } }));
            } catch { }
        ...
        if (name === "share_send") {
            try {
                window.dispatchEvent(new Event("sw:send"));
            } catch { } return;
        }
        if (name === "upload_lab_result") {
            try {
                window.dispatchEvent(new Event("labs:upload:open"));
            } catch { } onAgentMessage?.({ who: "system", msg: "Opened lab results uploader." }); return;
        }
    }
        ...
        <>
            <div className="voice-assistant" style={{ zIndex: 1000 }}>
                ...
                <div className="va-controls">
                    ...
                    <button
                        className="va-btn is-danger" onClick={stopAll} title="End session & reset">
                        <FaBroom />&nbsp;End Session
                    </button>
