...
/** Fallback tools (kept identical to backend schema) */
const CN_TOOLS_FALLBACK = [
    ...
    { name: "cn_save", description: "Ask UI to approve & save current clinical notes.", parameters: { type: "object", additionalProperties: false, properties: {} } },
    { name: "cn_preview", description: "Open preview tab for the clinical notes.", parameters: { type: "object", additionalProperties: false, properties: {} } },
];
...
export default function HelperAgent({
    ...
    const sendContext = async () => {
        ...
        } catch (e) { console.warn("sendContext error:", e); }
    };
    ...
    const sendSessionUpdate = async () => {
        ...
        const msg = {
            type: "session.update", session: {
                ...
                tools, tool_choice, instructions: instruction
            }
        };
    ...
    const stopAll = () => {
        ...
        if (remoteAudioEl.current) {
            try { remoteAudioEl.current.pause?.(); remoteAudioEl.current.srcObject = null; remoteAudioEl.current.src = ""; } catch { }
        }
    ...
    // Stronger speed boost while AI is speaking
    const orbBoost = vizSource === "agent" ? 2.0 : 1.0;
    ...
    return (
        <div className="helper-assistant" role="dialog" aria-label="Helper Agent">
            ...
            {/* Orb at the top (above ring/content) */}
            {/* Orb at the top (above ring/content) */}
            <div className="ha-orb">
                <div className="ha-orb-ring" aria-hidden="true" />
                <div className="ha-orb-clip">
                    {/* boost already makes acceleration very visible when AI speaks */}
                    <Orb className="ha-orb-canvas" boost={vizSource === "agent" ? 2.2 : 1.0} />
                </div>
            </div>
            ...
            <div className="ha-controls">
                ...
                <button className="ha-btn is-danger" onClick={() => { stopAll(); onClose?.(); }} title="End session & close">
                    <FaBroom />&nbsp;End
                </button>
