...
export default function SymptomsChecker({ sessionId, transcript, backendBase }) {
  ...
  useEffect(() => {
    if (!transcript || diagnoses.length > 0) return;
    fetchData();
  }, [transcript]);
  ...
  if (loading) return <div className="sc-loader">Analyzing symptoms…</div>;
    ...
    <div className="sc-root">
      ...
      <div className="sc-diagnosis-list">
        {diagnoses.map((d) => (
          ...
          >
            {d.name} <span className="sc-percent">{Math.round(d.likelihood_score * 100)}%</span>
          </motion.button>
        ))}
      </div>
          ...
          >
            ...
            <details className="sc-readmore">
              ...
              {selectedDiag.source && (
                <p className="sc-source">
                  Source:{" "}
                  <a href={selectedDiag.source.url} target="_blank" rel="noreferrer">
            ...
            <div className="sc-tree">
              {selectedDiag.symptoms.map((s, i) => (
                <motion.div
                  ...
                  style={{
                    "--weight": Math.max(2, s.weight * 10) + "px",
                    "--opacity": s.weight,
                  }}
                >
            ...
            {selectedDiag.questions?.length > 0 && (
              <div className="sc-questions">
                ...
                <button className="sc-refine" disabled={refining} onClick={refine}>
      ...
      <div className="sc-piechart" ref={pieRef}>
        {diagnoses.map((d) => (
          <div
            ...
            style={{
              flex: d.likelihood_score,
              backgroundColor: `hsl(${Math.round(d.likelihood_score * 120)},70%,50%)`,
            }}
            onClick={() => setSelected(d.id)}
          >
            <span>{d.name}</span>
